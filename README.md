...
endpoint and referenced secret. When `api_key_env` is set, the endpoint URL must
use HTTPS to prevent leaking the bearer token over cleartext HTTP. Hosted/server-side
submission should add URL and secret-reference policy before promoting this shape
into the API.
